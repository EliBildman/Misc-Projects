 ...
 * Author: Eli Bildman
 *
 * Description: takes an expression in a string and returns a string with the expression simplified
 *
...
public class ExpressionSimplifier 
{
		...
		for (int i = 0; i < str.length(); i ++) {
			if (str.charAt(i) == ' ') {
				if (str.substring(i + 1).indexOf(' ') != -1) {
					arr.add(str.substring(i + 1, str.substring(i + 1).indexOf(' ') + i + 1));
				} else {
	...
	private static String calculate(String left, String op, String right)
	{
		...
		double opr1 = Double.parseDouble(left);
		double opr2 = Double.parseDouble(right); 
		switch (op) {
			case "+":
				return opr1 + opr2 + "";
			case "-":
	...
	private static void replaceInList(ArrayList<String> items, int firstIndex, int howManyItems, String newValue)
	{
		...
		for (int i = firstIndex; i < howManyItems + firstIndex; i ++) {

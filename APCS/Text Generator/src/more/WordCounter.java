...
public class WordCounter implements TextAnalyzer
{
	...
	public WordCounter(String name) throws FileNotFoundException {		
		...
		while(fileScanner.hasNext()) {
			...
			if(startsWithCapitalLetter(token) && isSentenceEndingPunctuation(last)) {
	...
	public String organizedWords() {
		...
		for(int i = 0; i < words.size(); i ++) {
			for(String key: words.keySet()) {
				if (words.get(key) > biggest && !taken.contains(key)) {
					biggest = words.get(key);
					bigKey = key;
				}
			}
			end += "['" + bigKey + "': " + biggest + "], ";

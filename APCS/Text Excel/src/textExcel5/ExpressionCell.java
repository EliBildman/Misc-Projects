...
public class ExpressionCell extends Cell
{
	...
	public Double sum(String input) {
		Double sum = 0.0;
		for (int x = sheet.getCol(TextExcel.parseColon(input, 0)); x <= sheet.getCol(TextExcel.parseColon(input, 1)); x++){
			for (int y = sheet.getRow(TextExcel.parseColon(input, 0)); y <= sheet.getRow(TextExcel.parseColon(input, 1)); y++) {

...
public class CardGameGUI extends JFrame implements ActionListener {
	 ...
	 */
	public CardGameGUI(IBoard gameBoard) {
		board = gameBoard;
		totalWins = 0;
		totalGames = 0;
	...
	public void repaint() {
		for (int k = 0; k < board.getBoardSize(); k++) {
			String cardImageFileName =
				imageFileName(board.getCard(k), selections[k]);
		...
		statusMsg.setText(board.getCardsLeftInDeck()
	...
	private void initDisplay()	{
		...
		statusMsg = new JLabel(
			board.getCardsLeftInDeck() + " undealt cards remain.");
		panel.add(statusMsg);

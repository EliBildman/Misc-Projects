...
def turn(guessed, missed):
    ...
    if over:
        ...
        print "You Lose"
        print "Word Was:", word.upper()
        over = True
    return not over

while True:
    a = random.randint(1, len(possibleWords) - 1)
    #print possibleWords[a]
    word = possibleWords[a]
    inGame = True
    guessed = []
    missed = []
    tries = 0
    print
    play = raw_input("Play Game? (Y/N): ")
    if not play.upper() == "Y":
        print
        break
    while inGame:
        inGame = turn(guessed, missed)
    print_board(missed)
print "Bye"
